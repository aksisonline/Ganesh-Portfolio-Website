  ...
  return (
    <>
      <button
        ...
        onClick={() => toggleVideo()}
      >
        {isVideoPlayed ? <FiPause size={18} /> : <FiPlay size={18} />}
      </button>
      <div className="fixed left-0 top-0 h-full w-full overflow-hidden bg-white dark:bg-black">
        ...
        {isVideoPlayed && (
          <video src="/media/background.webm" loop autoPlay muted className={'absolute top-0 z-10 block h-full w-full object-cover'} />
        )}
      </div>
    </>
  )
