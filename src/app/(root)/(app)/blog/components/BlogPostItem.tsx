...
const BlogPostItem: React.FC<{ post: IPost }> = ({ post }) => {
  return (
    <Link
      href={'/blog/' + post.slug}
      className={`block bg-white/50 px-3 py-5 backdrop-blur dark:bg-black/80 md:px-5 ${post.metadata.highlight ? 'text-yellow-500' : ''}`}
    >
      ...
      <p className="text-sm opacity-60 dark:text-neutral-400">
        {post.metadata.summary ? post.metadata.summary : formatDate(post.metadata.publishedAt)}
      </p>
    </Link>
  )
}
